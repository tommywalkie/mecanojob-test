...
import { JS_DAY_TO_DAY_OF_WEEK, groupAvailabilitiesByDay } from '@/utils/date-utils'
...
function PublicAvailability() {
  ...
  const { data: availabilities, isLoading, error } = usePublicAvailability(userId || '')
  const [selectedDate, setSelectedDate] = useState<Date | null>(null)
  const [availableTimes, setAvailableTimes] = useState<{ start: Date; end: Date }[]>([])
  const [bookedSlots, setBookedSlots] = useState<BookedSlot[]>([])
  const navigate = useNavigate()

  // Group availabilities by day - memoized to prevent recalculation
  const availabilityByDay = useMemo(() => groupAvailabilitiesByDay(availabilities || []), [availabilities])
  ...
  if (isLoading) return <div className="text-center mt-10">Loading availability...</div>

  if (error) return <div className="text-center mt-10 text-red-500">Error loading availability</div>

  if (!availabilities?.length) {
    return <div className="text-center mt-10">No availability found for this user.</div>
  }
          ...
          {!availableTimes.length ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {availableTimes.map((slot, index) => {
                ...
                const slotStart = new Date(`${slotDate} ${formatTime(slot.start)}`)
                ...
                return (
                  <button
                    key={index}
                    onClick={() => !isBooked && handleTimeSelect(slot.start, slot.end)}
                    className={`
                      p-3 border border-gray-300 rounded text-center
                      ${
                        isBooked ? 'bg-gray-200 hover:bg-gray-200 text-gray-400 cursor-not-allowed' : 'hover:bg-gray-50'
                      }
                    `}
                  >
