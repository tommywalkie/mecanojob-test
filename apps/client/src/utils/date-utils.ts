...
export const sortDays = <T extends string>(days: T[]): T[] => {
  return days.sort(
    (a, b) =>
      DAYS_OF_WEEK.indexOf(a as unknown as DayOfWeek) -
      DAYS_OF_WEEK.indexOf(b as unknown as DayOfWeek)
  );
};
...
export const groupAvailabilitiesByDay = <T extends { day: string }>(
  availabilities: T[]
): Record<string, T[]> => {
  return (
